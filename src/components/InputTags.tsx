...
export const InputTags = React.forwardRef<HTMLInputElement, InputTagsProps>(
  (
    { placeholder = "", tags = [], renderTag = defaultRenderTag },
    forwardedRef
  ) => {
    ...
    const addValue = () => {
      setValues(prevValues => [...prevValues, fieldValue]);
      setFieldValue("");
    };
    const removeValue = (i: number) =>
      setValues(prevValues =>
        prevValues
          .slice(0, i)
          .concat(prevValues.slice(i + 1, prevValues.length))
      );
        ...
        {values.length > 0 && (
          <ChipsContainer>
            {values.map((tag, index) =>
              renderTag({ tag, removeTag: () => removeValue(index) })
            )}
          </ChipsContainer>
        )}
      </Container>
