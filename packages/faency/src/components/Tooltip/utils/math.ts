...
type calculateHorizontalPositionReturn = {
  overlay: number
  arrowStyle: Position
}
...
export const calculateVerticalPosition = ({
  ...
  preferredPosition,
}: CalculateVerticalPositionType): Position => {
  ...
  const positionIfAbove: Position = {
    height: heightIfAbove - verticalMargins,
    top: activatorTop + containerRectTop - heightIfAbove,
    positioning: 'top',
  }

  const positionIfBelow: Position = {
...
export const calculateHorizontalPosition = ({
  ...
  overlayMargins,
  preferredAlignment,
}: CalculateHorizontalPositionType): calculateHorizontalPositionReturn => {
  const maximum = containerRect.width - overlayRect.width

  let overlayHorizontalPosition

  if (preferredAlignment === 'left') {
    overlayHorizontalPosition = Math.min(maximum, Math.max(0, activatorRect.left - overlayMargins.horizontal))
  } else if (preferredAlignment === 'right') {
    const activatorRight = activatorRect.left + activatorRect.width
    overlayHorizontalPosition = Math.min(
      maximum,
      Math.max(0, activatorRight - overlayRect.width + overlayMargins.horizontal),
    )
  } else {
    overlayHorizontalPosition = Math.min(maximum, Math.max(0, activatorRect.center.x - overlayRect.width / 2))
  }
  ...
  const arrowStyleByOverlayAlignment: StyleByAlignmentType = {
    ...
    right: {
      right: overlayHorizontalPosition ? 10 : `calc(100% - ${activatorRect.width + activatorRect.left - 10}px)`,
    },
  }
